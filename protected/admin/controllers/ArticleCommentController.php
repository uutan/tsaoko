...
class ArticleCommentController extends Controller
{
	...
	{
		$form = new Form('backend.views.articleComment.formConfig');
	    $form->model = new ArticleComment;
	    ...
	    if($form->submitted('btnsubmit'))
	    {
			...
			*/
	    	...
	    	if($form->validate())
	    	{
		        ...
		        if($model->save(false))
		        {
		        	Yii::app()->user->setFlash('success', ArticleComment::model()->modelName."“{$model}”已成功添加！");
		...
		if($form->submitted('btnsubmit'))
	    {
	    	...
	    	if($form->validate())
	    	{
				...
				/**
		        	...
		        	*/		        	
		        	Yii::app()->user->setFlash('success',ArticleComment::model()->modelName."“{$model}”已成功修改！");
					$this->redirect(array('view','id'=>$model->id));
				}
	    	}
			/**
	...
	public function actionOperate()
	{
		if(Yii::app()->request->isAjaxRequest && !empty($_POST['id']))
		{
			...
			{
				$objs = ArticleComment::model()->findAllByPk($_POST['id']);
				if($objs)
				{
					foreach($objs as $obj)
					{
	...
	public function loadModel()
	{
		...
		{
			if(isset($_GET['id']))
				$this->_model=ArticleComment::model()->findbyPk($_GET['id']);
			if($this->_model===null)
