...
class ArticleCategoryController extends Controller
{
	...
	{
		$form = new Form('backend.views.articleCategory.formConfig');
	    $form->model = new ArticleCategory;
	    ...
	    if($form->submitted('btnsubmit'))
	    {
			...
			*/
	    	...
	    	if($form->validate())
	    	{
		        ...
		        if($model->save(false))
		        {
		        	Yii::app()->user->setFlash('success', ArticleCategory::model()->modelName."“{$model}”已成功添加！");
		...
		if($form->submitted('btnsubmit'))
	    {
	    	...
	    	if($form->validate())
	    	{
				...
				/**
		        	...
		        	*/		        	
		        	Yii::app()->user->setFlash('success',ArticleCategory::model()->modelName."“{$model}”已成功修改！");
					$this->redirect(array('view','id'=>$model->id));
				}
	    	}
			/**
	...
	public function actionOperate()
	{
		if(Yii::app()->request->isAjaxRequest && !empty($_POST['id']))
		{
			...
			{
				$objs = ArticleCategory::model()->findAllByPk($_POST['id']);
				if($objs)
				{
					foreach($objs as $obj)
					{
