...
class AnnController extends Controller
{
	...
	{
		$form = new Form('backend.views.ann.formConfig');
	    $form->model = new Ann;
	    ...
	    if($form->submitted('btnsubmit'))
	    {
			...
			*/
	    	...
	    	if($form->validate())
	    	{
		        ...
		        if($model->save(false))
		        {
		        	Yii::app()->user->setFlash('success', Ann::model()->modelName."“{$model}”已成功添加！");
		...
		if($form->submitted('btnsubmit'))
	    {
	    	...
	    	if($form->validate())
	    	{
				...
				/**
		        	...
		        	*/		        	
		        	Yii::app()->user->setFlash('success',Ann::model()->modelName."“{$model}”已成功修改！");
					$this->redirect(array('view','id'=>$model->id));
				}
	    	}
			/**
	...
	{
		$model=new Ann('search');
		$model->unsetAttributes();
		if(isset($_GET['Ann']))
			$model->attributes=$_GET['Ann'];
	...
	public function actionOperate()
	{
		if(Yii::app()->request->isAjaxRequest && !empty($_POST['id']))
		{
			...
			{
				$objs = Ann::model()->findAllByPk($_POST['id']);
				if($objs)
				{
					foreach($objs as $obj)
					{
	...
	public function loadModel()
	{
		...
		{
			if(isset($_GET['id']))
				$this->_model=Ann::model()->findbyPk($_GET['id']);
			if($this->_model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='ann-form')
		{
