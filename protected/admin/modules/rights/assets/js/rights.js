...
(function($) {
	...
	*/
	$(document).ready(function() {
		...
		*/
   		$('#rights .flash').animate({ opacity: 1.0 }, { duration: 3000 }).fadeOut(650);

   		var navbar_prepended = false;
		var $navbar = $("#content .grid-view table.items");

		if($navbar.size()>0){
		    var start_y = $navbar.offset().top;
		    var navbar = $navbar.clone();
		    navbar.width($navbar.width());
		    navbar.find('tbody').remove();
		    navbar.css('z-index', 99999999);
		    navbar.addClass('fixed-thead');

		    navbar.find('th').each(function(i, n){
		    	$(this).width($navbar.find('th').eq(i).width());
		    });
